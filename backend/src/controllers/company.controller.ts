...
export default class CompanyController {
  ...
  async getACompany(@Param('id') id: number, @Req() req: Request) {
    try {
      const userid = Number(req['key'].userid);
      const response = await this.companyservice.getACompany(
        Number(id),
        userid,
      );
      if (response) {
  ...
  @HttpCode(200)
  @Patch('/:id')
  async updateACompany(
    @Param('id') id: number,
    @Body() company: CreateCompanyDTO,
  ) {
    try {
      const response = await this.companyservice.updateACompany(
        Number(id),
        company,
      );
      if (response) {
